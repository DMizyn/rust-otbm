...
fn test_basic_attributes() {
    ...
    item.set_attribute(ItemAttribute::Name, AttributeValue::String("Test Item".to_string()));
    item.set_attribute(ItemAttribute::Description, AttributeValue::String("A test item".to_string()));
...
fn test_position_attribute() {
    ...
    item.set_attribute(
        ...
        AttributeValue::Position { x: 100, y: 200, z: 7 }
    );
...
fn test_color_attribute() {
    ...
    item.set_attribute(
        ...
        AttributeValue::Color { r: 255, g: 128, b: 64 }
    );
...
fn test_custom_attributes() {
    ...
    if let Some(AttributeValue::Integer(level)) = item.get_attribute(&ItemAttribute::Custom("enchantment".to_string())) {
        assert_eq!(*level, 3);
    } else {
        panic!("Expected enchantment attribute");
    }
    
    if let Some(AttributeValue::String(creator)) = item.get_attribute(&ItemAttribute::Custom("creator".to_string())) {
