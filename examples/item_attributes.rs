...
fn main() {
    ...
    item.set_attribute(ItemAttribute::Description, AttributeValue::String("A powerful magical sword".to_string()));
    ...
    item.set_attribute(
        ...
        AttributeValue::Position { x: 1000, y: 1000, z: 7 }
    );
    ...
    if let Some(AttributeValue::Integer(level)) = item.get_attribute(&ItemAttribute::Custom("enchantment_level".to_string())) {
